...
# Load gif paths
gif_paths = {
    'idle': 'hamster1_idle_framework.gif',
    'idle_to_sleep': 'hamster1_idle_to_sleep_framework.gif',
    'sleep': 'hamster1_sleep_framework.gif',
    'sleep_to_idle': 'hamster1_sleep_to_idle_framework.gif',
    'walk_positive': 'hamster1_running_left_framework.gif',
    'walk_negative': 'hamster1_running_right_framework.gif',
    'love': 'hamster1_nibbling_framework.gif',
    'dragging': 'hamster1_dragging_framework.gif',
    'falling': 'hamster1_falling_framework.gif'
}
...
def apply_physics():
    global pet_position, last_x, last_y
    ...
    if pet_position['y'] >= (1080 - TASKBAR_HEIGHT):
    ...
    window.after(10, apply_physics)  # Keep calling apply_physics
...
def play_falling_animation():
    ...
    if is_falling:
        frame = falling[cycle % len(falling)]
        cycle += 1
        label.configure(image=frame)
        window.after(100, play_falling_animation)
        print(pet_position['y'], is_falling, " di pa ako nasa taskbar")
        if pet_position['y'] > (1080 - TASKBAR_HEIGHT) - 50:
            print(pet_position['y'], is_falling,
                  " nasa taskbar na ako arf arf")
            pet_position['y_velocity'] = 0
            is_falling = False
            cycle = 0
    else:
...
def show_drag_animation():
    ...
    if is_drag_animation_running:
        # Ensure the cycle value is within the range of dragging frames
        cycle %= len(dragging)
        ...
        # If not dragging, reset the dragging animation state and cycle
...
idle = [tk.PhotoImage(file=resource_path(os.path.join(
    script_directory, gif_paths['idle'])), format=f'gif -index {i}') for i in range(4)]
idle_to_sleep = [tk.PhotoImage(file=resource_path(os.path.join(
    script_directory, gif_paths['idle_to_sleep'])), format=f'gif -index {i}') for i in range(7)]
sleep = [tk.PhotoImage(file=resource_path(os.path.join(
    script_directory, gif_paths['sleep'])), format=f'gif -index {i}') for i in range(7)]
sleep_to_idle = [tk.PhotoImage(file=resource_path(os.path.join(
    script_directory, gif_paths['sleep_to_idle'])), format=f'gif -index {i}') for i in range(7)]
walk_positive = [tk.PhotoImage(file=resource_path(os.path.join(
    script_directory, gif_paths['walk_positive'])), format=f'gif -index {i}') for i in range(4)]
walk_negative = [tk.PhotoImage(file=resource_path(os.path.join(
    script_directory, gif_paths['walk_negative'])), format=f'gif -index {i}') for i in range(4)]
love_frames = [tk.PhotoImage(file=resource_path(os.path.join(
    script_directory, gif_paths['love'])), format=f'gif -index {i}') for i in range(2)]
dragging = [tk.PhotoImage(file=resource_path(os.path.join(
    script_directory, gif_paths['dragging'])), format=f'gif -index {i}') for i in range(9)]
falling = [tk.PhotoImage(file=resource_path(os.path.join(
    script_directory, gif_paths['falling'])), format=f'gif -index {i}') for i in range(2)]
