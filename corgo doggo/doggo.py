...
def show_drag_animation():
    global cycle, event_number, is_drag_animation_running, drag_animation_complete
    ...
    if is_drag_animation_running:
        ...
        if cycle < len(dragging) - 1:
            cycle += 1
        elif cycle == len(dragging) - 1:
            # If it's the last frame of the dragging animation, freeze on this frame
            drag_animation_complete = True
        else:
            is_drag_animation_running = False

        if not drag_animation_complete:
            window.after(100, show_drag_animation)
        else:
            # Once dragging animation is complete, revert to normal update cycle
            update()
    else:
